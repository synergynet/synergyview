	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (object instanceof AnnotationSetNode) {
			...
			InputDialog dialog = new InputDialog(PlatformUI.getWorkbench()
					.getDisplay().getActiveShell(), "Rename Annoatation Set",
					"Enter Annotation Set Name:", node.getResource().getName(),
					validator);
			if (dialog.open() == Window.OK) {
				try {
					...
					IStatus status = new Status(IStatus.ERROR,
							Activator.PLUGIN_ID,
							"Unable to rename the Annotation Set", e);
					logger.log(status);
					MessageDialog.openError(PlatformUI.getWorkbench()
							.getDisplay().getActiveShell(), "Save Error",
							"Unable to rename the Annotation Set!");
				}
			} else {
				return null;
			}
