...
public class AnnotationIntervalImpl extends IntervalImpl {
	 ...
	 *            the owner
	 */
	public AnnotationIntervalImpl(AnnotationSetNode annotationSetNode,
			JaretDate begin, JaretDate end, IntervalAnnotation annotationValue,
			TimeBarMarkerImpl marker, SubjectRowModel owner) {
		super(begin, end);
		ownerRow = owner;
				...
				if (isMarkerInrange()) {
					if (captionState == CaptionPublishState.UNSET) {
						...
						for (ICaptionChangeListener captinListener : captionChangeListeners) {
							CaptionChangeEvent event = new CaptionChangeEvent(
									AnnotationIntervalImpl.this,
									annotation.getText(), captionState);
							captinListener.captionChange(
									AnnotationIntervalImpl.this, event);
						}
					}
				} else {
		...
		annotation.addPropertyChangeListener(Annotation.PROP_TEXT,
				new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent arg0) {
						AnnotationIntervalImpl.this.setLabel((String) arg0
								.getNewValue());
					}
	 ...
	 */
	public void addCaptionChangeListener(
	...
	public boolean isMarkerInrange() {
		if ((marker.getDate().compareTo(this.getBegin()) >= 0)
				&& (marker.getDate().compareTo(this.getEnd()) < 0)) {
			return true;
		} else {
			return false;
		}
	 ...
	 *            the caption change listener
	 */
	public void removeCaptionChangeListener(
			ICaptionChangeListener captionChangeListener) {
	 ...
	 * 
	 * @see
	 * de.jaret.util.date.IntervalImpl#setBegin(de.jaret.util.date.JaretDate)
	 */
	@Override
	public void setBegin(JaretDate begin) {
		...
		annotation.setStartTime(DateTimeHelper.getMilliFromJaretDate(this
				.getBegin()));
	}
	...
	public void setLabel(String label) {
		this.firePropertyChange(AnnotationIntervalImpl.PROP_LABLE, this.label,
				this.label = label);
	}
