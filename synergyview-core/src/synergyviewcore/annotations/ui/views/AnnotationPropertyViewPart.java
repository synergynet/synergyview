...
public class AnnotationPropertyViewPart extends ViewPart implements ISelectionListener, CollectionChangeListener {
	...
	public void update(ViewerCell cell) {
		...
		cell.setImage((Image) resourceManager.get(ResourceLoader.getIconDescriptor(AttributeNode.ATTRIBUTE_ICON)));
	    }
	...
	for (Attribute attribute : attributeList) {
	    attribute.removePropertyChangeListener(attributePropertyChangeListener);
	}
	attributeList.clear();
	if (!attributeTableViewer.getControl().isDisposed() && (attributeTableViewer.getContentProvider() != null) && (attributeTableViewer.getInput() != null)) {
	...
	annotationText = new Text(composite, SWT.WRAP | SWT.MULTI | SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL | SWT.READ_ONLY);
	annotationText.setBackground(new Color(Display.getDefault(), 0, 0, 0));
	annotationText.setBackground(Display.getDefault().getSystemColor(SWT.COLOR_BLACK));
	annotationText.setForeground(Display.getDefault().getSystemColor(SWT.COLOR_YELLOW));
	...
	attributeTableViewer = new TableViewer(composite, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.FULL_SELECTION);
	attributeTableViewer.getControl().setLayoutData(viewerGridData);
	TableViewerColumn c = new TableViewerColumn(attributeTableViewer, SWT.NONE);
	c.getColumn().setText("Name");
	c.setLabelProvider(new AttributeTextLabelProvider());
	c.getColumn().setWidth(200);
	TableViewerColumn c2 = new TableViewerColumn(attributeTableViewer, SWT.NONE);
	...
	attributeTableViewer.setContentProvider(new AttributesContentProvider());
	...
	attributePropertyChangeListener = new PropertyChangeListener() {
		...
		if (evt.getSource() instanceof Attribute) {
		    AnnotationPropertyViewPart.this.attributeTableViewer.refresh(evt.getSource());
		}
	    }
	...
	for (CollectionDiffEntry<?> entry : event.getListDiff().getDifferences()) {
		...
		attribute.addPropertyChangeListener(attributePropertyChangeListener);
	    } else {
	...
	if ((part instanceof CollectionMediaClipAnnotationEditor) && (selection instanceof IStructuredSelection)) {
	    if (selection.isEmpty() || (!(((IStructuredSelection) selection).getFirstElement() instanceof AnnotationIntervalImpl))) {
		clearData();
		return;
	    }
	    AnnotationIntervalImpl selectedInterval = (AnnotationIntervalImpl) ((IStructuredSelection) selection).getFirstElement();
	    annotation = selectedInterval.getAnnotation();
	    projectAttributeRootNode = ((ProjectNode) selectedInterval.getOwner().getAnnotationSetNode().getLastParent()).getProjectAttributeRootNode();
	    AnnotationAttributeController selectedController = selectedInterval.getOwner().getAnnotationSetNode().getAnnotationAttributeController(annotation);
	...
	for (Attribute attribute : controller.getAttributeList()) {
		...
		Attribute newAttribute = projectAttributeRootNode.getAttribute(attribute);
		newAttribute.addPropertyChangeListener(attributePropertyChangeListener);
	    }
	}
    }
