...
public class AnnotationTableViewPart extends ViewPart implements
	...
	private static class AnnotationsContentProvider implements
		...
		private CollectionChangeListener listener = new CollectionChangeListener() {
			public void listChanged(CollectionChangeEvent event) {
				CollectionDiffEntry<?>[] differences = event.getListDiff()
						.getDifferences();
				for (int i = 0; i < differences.length; i++) {
					...
					if (entry.isAddition()) {
						...
						if (viewer instanceof AbstractListViewer) {
							((AbstractListViewer) viewer).add(entry
									.getElement());
						} else {
							((TableViewer) viewer).insert(entry.getElement(),
									entry.getPosition());
						}
					} else {
						
						if (viewer instanceof AbstractListViewer) {
							((AbstractListViewer) viewer).remove(entry
									.getElement());
						} else {
		...
		public AnnotationsContentProvider() {
			knownElements = new WritableSet(SWTObservables.getRealm(Display
					.getDefault()));
		}
		...
		public Object[] getElements(Object inputElement) {
			return contentList != null ? contentList.toArray()
					: new Object[] {};
		}
		...
		public void inputChanged(Viewer viewer, Object oldInput, Object newInput) {
			this.viewer = viewer;
			if ((newInput != null)
					&& (!(newInput instanceof AnnotationSetNode))) {
				throw new IllegalArgumentException(
						"This content provider only works with input of type ObservableList");
			}
			...
			knownElements.clear();
			if (newInput != null) {
				contentList = (List<?>) ((AnnotationSetNode) newInput)
						.getAnnotationList();
	...
	/**
	 * The listener interface for receiving attributeDrop events. The class that
	 * is interested in processing a attributeDrop event implements this
	 * interface, and the object created with that class is registered with a
	 * component using the component's
	 * <code>addAttributeDropListener<code> method. When
	...
	private static class AttributeDropListener extends ViewerDropAdapter {
		 ...
		 * 
		 * @see
		 * org.eclipse.jface.viewers.ViewerDropAdapter#dragOver(org.eclipse.
		 ...
		 * 
		 * @see
		 * org.eclipse.jface.viewers.ViewerDropAdapter#drop(org.eclipse.swt.
		 * dnd.DropTargetEvent)
		 */
		@Override
					...
					if (!selection.isEmpty()) {
						...
						while (i.hasNext()) {
							...
							if (o instanceof AttributeNode) {
								...
								if (node != null) {
									node.getAnnotationAttributeController(
											dropTarget).addAttributeList(list);
								}
							}
						}
					}
	...
	public void createPartControl(Composite parent) {
		...
		annotationSetTableViewer
				.setContentProvider(new AnnotationsContentProvider());
		...
		annotationSetTableViewer.addDropSupport(operations, transferTypes,
				dropListener);
			...
			public void partActivated(IWorkbenchPartReference partRef) {
				if (partRef.getId().compareTo(
					...
					if (currentAnnotationSetNode == null) {
						currentAnnotationSetNode = editor
		...
		getSite().getWorkbenchWindow().getPartService()
				.addPartListener(activeEditorPartListener);
	...
	public void dispose() {
		getSite().getWorkbenchWindow().getPartService()
				.removePartListener(activeEditorPartListener);
		clearData();
		super.dispose();
	}
	...
	public void selectionChanged(IWorkbenchPart part, ISelection selection) {
		if (part instanceof CollectionMediaClipAnnotationEditor) {
			if (selection instanceof StructuredSelection) {
				...
				sSelection.iterator();
				if ((sSelection.getFirstElement() != null)
						&& (sSelection.getFirstElement() instanceof AnnotationIntervalImpl)) {
					AnnotationIntervalImpl interval = (AnnotationIntervalImpl) sSelection
							.getFirstElement();
					StructuredSelection selfSelection = (StructuredSelection) annotationSetTableViewer
							.getSelection();
					if ((selfSelection != null)
							&& (selfSelection.getFirstElement() != null)) {
						if (!selfSelection.getFirstElement().equals(
								interval.getAnnotation())) {
							IStructuredSelection newSelection = new StructuredSelection(
									interval.getAnnotation());
							annotationSetTableViewer.setSelection(newSelection,
									true);
						}
					} else {
						IStructuredSelection newSelection = new StructuredSelection(
