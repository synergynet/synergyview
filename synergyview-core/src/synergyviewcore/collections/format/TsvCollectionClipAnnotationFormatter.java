/**
 * File: CsvCollectionClipAnnotationFormatter.java Copyright (c) 2011 phyo This
...
public class TsvCollectionClipAnnotationFormatter implements
	 ...
	 * 
	 * @see
	 * synergyviewcore.collections.format.ICollectionClipAnnotationFormatter
	 * #export(java.util.List, java.io.OutputStream)
	 */
		...
		try {
			...
			for (AnnotationSet set : clipsToExport.getAnnotationSetList()) {
				...
				str.append(String.format("%s - %s (%s)\n", set.getName(), set
						.getCollectionMediaClip().getClipName(), set
						.getCollectionMediaClip().getCollection().getName()));
				str.append(" \n");
				str.append(String.format("Clip Duration:\t%s\n", DateTimeHelper
						.getHMSFromMilliFormatted(set.getCollectionMediaClip()
								.getDuration())));
				str.append(" \n");
				...
				for (Annotation annotation : annotationList) {
					SubjectAnnotationCount subjectAnnotationCount = subjectMap
							.get(annotation.getSubject());
					if (subjectAnnotationCount == null) {
						subjectAnnotationCount = new SubjectAnnotationCount();
						subjectMap.put(annotation.getSubject(),
								subjectAnnotationCount);
					}
					...
					if (annotation instanceof IntervalAnnotation) {
						...
						subjectAnnotationCount
								.addTotalMilliSec(intervalAnnotation
										.getDuration());
						StringBuilder attributes = new StringBuilder();
						
						List<Attribute> attributesList = intervalAnnotation
								.getAttributes();
						...
						for (int i = 0; i < attributesList.size(); i++) {
							...
							if (!subjectAnnotationCount.getAttributeDuration()
									.containsKey(attributesList.get(i))) {
								AttributeInfo info = new AttributeInfo();
								info.setTotalMilliSec(intervalAnnotation
										.getDuration());
								info.incrementCount();
								subjectAnnotationCount.getAttributeDuration()
										.put(attributesList.get(i), info);
							} else {
								AttributeInfo info = subjectAnnotationCount
										.getAttributeDuration().get(
												attributesList.get(i));
								info.setTotalMilliSec(info.getTotalMilliSec()
										+ intervalAnnotation.getDuration());
								info.incrementCount();
								subjectAnnotationCount.getAttributeDuration()
										.put(attributesList.get(i), info);
							}
						...
						if (!attributesStr.isEmpty()) {
							attributesStr = attributesStr.substring(0,
									attributesStr.length() - 2);
						}
						formatedLine = String.format("%s\t%s\t%s\t%s\t%s\n",
								intervalAnnotation.getFormattedStartTime(),
								intervalAnnotation.getFormattedDuration(),
								intervalAnnotation.getSubject().getName(), txt,
								attributesStr);
					} else {
						formatedLine = String.format("%s\t-\t%s\t", annotation
								.getFormattedStartTime(), annotation
								.getSubject().getName())
								+ txt + "\n";
					}
				...
				str.append("Summary\n");
				str.append("--------------------------------------------\n");
				for (Map.Entry<Subject, SubjectAnnotationCount> entry : subjectMap
						.entrySet()) {
					str.append("Subject:\t" + entry.getKey().getName() + "\n");
					str.append("Annotation count:\t"
							+ entry.getValue().getCount() + "\n");
					str.append(String.format("Total duration:\t%s\n",
							DateTimeHelper.getHMSFromMilliFormatted(entry
									.getValue().getTotalMilliSec())));
					str.append(" \n");
					str.append("Attribute\tOccurrences\tTotal Time\n");
					for (Map.Entry<Attribute, AttributeInfo> attributeEntry : entry
							.getValue().getAttributeDuration().entrySet()) {
						str.append(String.format("%s\t%d\t%s\n", attributeEntry
								.getKey().getName(), attributeEntry.getValue()
								.getCount(), DateTimeHelper
								.getHMSFromMilliFormatted(attributeEntry
										.getValue().getTotalMilliSec())));
					}
