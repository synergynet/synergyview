 ...
 * @author phyo
 */
public class TsvCollectionClipAnnotationFormatter implements ICollectionClipAnnotationFormatter {
	...
	try {
		...
		for (Annotation annotation : annotationList) {
			...
			if (!attributesStr.isEmpty()) {
			    attributesStr = attributesStr.substring(0, attributesStr.length() - 2);
			}
			formatedLine = String.format("%s\t%s\t%s\t%s\t%s\n", intervalAnnotation.getFormattedStartTime(), intervalAnnotation.getFormattedDuration(), intervalAnnotation.getSubject().getName(), txt, attributesStr);
		    } else {
		...
		for (Map.Entry<Subject, SubjectAnnotationCount> entry : subjectMap.entrySet()) {
			...
			str.append(String.format("%s\t%d\t%s\n", attributeEntry.getKey().getName(), attributeEntry.getValue().getCount(), DateTimeHelper.getHMSFromMilliFormatted(attributeEntry.getValue().getTotalMilliSec())));
		    }
