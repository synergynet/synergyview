/**
 * File: CollectionFolder.java Copyright (c) 2010 phyo This program is free
...
public class CollectionRootNode extends AbstractParent<Collection> {
	...
	public List<String> getChildrenNames() {
		...
		for (INode collectionNode : _children) {
			nameList.add(((CollectionNode) collectionNode).getResource()
					.getName());
		}
		return nameList;
	...
	private void loadChildNodes() {
		...
		try {
			entityManager = _eManagerFactory.createEntityManager();
			TypedQuery<Collection> query = entityManager.createQuery(
					"select C from Collection C", Collection.class);
			List<Collection> result = query.getResultList();
			if (result.size() > 0) {
				for (Collection collectionItem : result) {
					CollectionNode collectionNodeFolder = new CollectionNode(
							collectionItem, this);
	 ...
	 * 
	 * @param collectionFolderValue
	 *            the collection folder value
	 */
	public void removeChildCollectionNode(CollectionNode collectionFolderValue) {
		...
		try {
			entityManager = _eManagerFactory.createEntityManager();
			entityManager.getTransaction().begin();
			Collection c = entityManager.merge(collectionFolderValue
					.getResource());
