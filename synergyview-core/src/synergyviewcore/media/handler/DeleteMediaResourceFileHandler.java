...
public class DeleteMediaResourceFileHandler extends AbstractHandler implements
		IHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		List<IFile> mediaDataItemListToRemove = new ArrayList<IFile>();
		for (Iterator<?> i = structSel.iterator(); i.hasNext();) {
			...
			if (element instanceof MediaNode) {
				try {
					...
					IFile file = media.getMediaFileResource();
					if (file!=null)
						mediaDataItemListToRemove.add(file);
				} catch (Exception e) {
		...
		try {
			OpenedProjectController.getInstance().getMediaController().deleteMediaResource(mediaDataItemListToRemove);
