...
// TODO Save and loading not working.

// TODO Fix bug where bar passes over top but video doesn't start playing.
...
public class VLCMedia extends AbstractMedia {
    ...
    /**Amount of time at start and end of clips that should stop events triggering as per normal playback.  */
    private static final float TIME_LIMIT_TOLERANCE = 0.0001f;
	...
	jPanel = new JPanel(new GridLayout()) {
		...
		if (pos > TIME_LIMIT_TOLERANCE && pos < 1 - TIME_LIMIT_TOLERANCE) {

		    // Update timebar location
     ...
     */
    public void dispose() {
	mediaPlayerComponent.release(); // TODO Fix bug where sometimes closing all the VLC instances at once crashes java.
    }
