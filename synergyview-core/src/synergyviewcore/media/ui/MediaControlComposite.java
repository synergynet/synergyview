...
class MarkerIntervalRenderer extends RendererBase implements TimeBarRenderer,
		TimeBarRenderer2 {
...
public class MediaControlComposite extends Composite {
	...
	private LocalResourceManager _resourceManager = new LocalResourceManager(
	...
	public MediaControlComposite(Composite parent, int style) {
		...
		_ctx.bindValue(SWTObservables.observeEnabled(_playPulse),
				SWTObservables.observeVisible(_timeBarViewer));
		_ctx.bindValue(SWTObservables.observeEnabled(_playPulse),
				SWTObservables.observeVisible(_timeScale));
	}
	...
	private void createMediaConstrols() {
		...
		_playPulse.setImage((Image) _resourceManager.get(ResourceLoader
				.getIconDescriptor("control_play_blue.png")));
	...
	private void createMediaTimeScaler() {
		...
		_timeScale.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				_timeBarViewer.setPixelPerSecond((double) _timeScale
						.getSelection() / 1000);
			}
		});
	}
	...
	private void createMediaTimeSlider() {
		...
		_timeBarViewer.addTimeBarChangeListener(new ITimeBarChangeListener() {
			...
			public void intervalIntermediateChange(TimeBarRow arg0,
					Interval arg1, JaretDate arg2, JaretDate arg3) {
			}
		...
		_timeBarViewer.registerTimeBarRenderer(MarkerInterval.class,
				new MarkerIntervalRenderer());
	...
	public void registerMedia(AbstractMedia media) {
		...
		_interval = new IntervalImpl(_startDate.copy(), _startDate.copy()
				.advanceMillis(media.getDuration()));
			...
			public void propertyChange(PropertyChangeEvent evt) {
				...
				_timeBarViewer.getDisplay().asyncExec(new Runnable() {
					public void run() {
						...
						if (!_isMarkerDragging) {
							_marker.setDate(_startDate.copy().advanceMillis(
									(long) _media.getTime()));
						}
					}
		...
		updateScale();
		_mediaBinding = _ctx.bindValue(
				SWTObservables.observeSelection(_playPulse),
	...
	private void updateScale() {
		_timeBarViewer.setPixelPerSecond((double) (((Composite) _timeBarViewer)
				.getClientArea().width * 1000)
				/ (double) (_media.getDuration()));
		_timeScale
				.setMaximum((int) (_timeBarViewer.getPixelPerSecond() * 1000 * SCALE_ZOOM_TIMES));
		_timeScale
				.setMinimum((int) (_timeBarViewer.getPixelPerSecond() * 1000));
		_timeScale
				.setSelection((int) (_timeBarViewer.getPixelPerSecond() * 1000));
	}
