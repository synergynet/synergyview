...
/**
 * Customised Interval Renderer that allows developers to define
 * gradient-enabled background colour.
 */

public class TimeBarIntervalRenderer extends RendererBase implements
	...
	public void draw(GC gc, Rectangle drawingArea,
		...
		if (_rounding == 0) {
			...
			gc.fillRoundRectangle(iRect.x, iRect.y, iRect.width, iRect.height,
					_rounding, _rounding);
			gc.drawRoundRectangle(iRect.x, iRect.y, iRect.width, iRect.height,
					_rounding, _rounding);
		}
		if (horizontal) {
		...
		if (muteImage == null) {
			muteImage = new Image(gc.getDevice(),
					TimeBarIntervalRenderer.class
							.getResourceAsStream("sound_mute.png"));
		}
