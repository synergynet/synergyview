 ...
 * 
 * @author Peter Kliem
 * @version $Id: CollapseNodeAction.java 160 2007-01-02 22:02:40Z olk $
 */
public class CollapseNodeAction extends Action {
     ...
     */
    public CollapseNodeAction(TimeBarViewer viewer, TimeBarViewerDelegate delegate) {
        _viewer = viewer;
        _delegate = delegate;
    }
    ...
    public void run() {
        if (_delegate.getHierarchicalViewState() != null) {
            TimeBarNode node = (TimeBarNode) _delegate.getCtxRow();
            if (node != null) {
                _delegate.getHierarchicalViewState().setExpandedRecursive(node, false);
            }
        }
    }
