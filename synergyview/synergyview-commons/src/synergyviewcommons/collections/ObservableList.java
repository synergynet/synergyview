 ...
 */
public class ObservableList<R extends List<E>, E> implements
		IObservableList<R, E> {
	/** The list. */
	private R list = null;
	...
	public boolean add(E element) {
		...
		CollectionDiffEntry<E>[] entries = createCollectionDiffEntryArray(
				element, true);
		eventNotify(entries);
		return true;
	}
	 ...
	 * 
	 * @see
	 * synergyviewcommons.collections.ICollectionObservable#addChangeListener
	 * (synergyviewcommons.collections.CollectionChangeListener)
	 */
	@Override
	...
	@Override
	public void clear() {
		CollectionDiffEntry<E>[] entries = createCollectionDiffEntryArray(list,
				false);
		list.clear();
		eventNotify(entries);
	}
	...
	private CollectionDiffEntry<E>[] createCollectionDiffEntryArray(
			Collection<?> collection, boolean isAddition) {
		CollectionDiffEntry<E>[] entries = (CollectionDiffEntry<E>[]) Array
				.newInstance(CollectionDiffEntry.class, collection.size());
		Iterator<?> iterator = collection.iterator();
		for (int i = 0; i < collection.size(); i++) {
			E item = (E) iterator.next();
			entries[i] = new CollectionDiffEntryImpl<E>(item, isAddition,
					list.indexOf(item));
		}
		return entries;
	}
	...
	@SuppressWarnings("unchecked")
	private CollectionDiffEntry<E>[] createCollectionDiffEntryArray(
			Object element, boolean isAddition) {
		CollectionDiffEntry<E>[] entries = (CollectionDiffEntry<E>[]) Array
				.newInstance(CollectionDiffEntry.class, 1);
		entries[0] = new CollectionDiffEntryImpl<E>((E) element, isAddition,
	 ...
	 * 
	 * @see
	 * synergyviewcommons.collections.ICollectionObservable#removeChangeListener
	 * (synergyviewcommons.collections.CollectionChangeListener)
	 */
	@Override
