 ...
 * @author phyokyaw
 */
public class AnnotationIntervalRender extends RendererBase implements
		TimeBarRenderer, TimeBarRenderer2, ISelectionListener {
	 ...
	 * 
	 * @see
	 * de.jaret.util.ui.timebars.swt.renderer.TimeBarRenderer#contains(de.jaret
	 * .util.date.Interval, org.eclipse.swt.graphics.Rectangle, int, int,
	 * boolean)
	 */
	 ...
	 * 
	 * @see
	 * de.jaret.util.ui.timebars.swt.renderer.TimeBarRenderer#createPrintrenderer
	 * (org.eclipse.swt.printing.Printer)
	 */
	public TimeBarRenderer createPrintrenderer(Printer printer) {
		AnnotationIntervalRender renderer = new AnnotationIntervalRender(
				printer);
		return renderer;
	}
	 ...
	 * 
	 * @see
	 * de.jaret.util.ui.timebars.swt.renderer.TimeBarRenderer#draw(org.eclipse
	 * .swt.graphics.GC, org.eclipse.swt.graphics.Rectangle,
	 * de.jaret.util.ui.timebars.TimeBarViewerDelegate,
	 * de.jaret.util.date.Interval, boolean, boolean, boolean)
	 */
	public void draw(GC gc, Rectangle drawingArea,
		...
		if (showAttributeSelection) {
			if (interval instanceof AnnotationIntervalImpl) {
				...
				for (Attribute attribute : annotationIntervalImpl
						.getAnnotation().getAttributes()) {
					if (selectedAttributes.contains(attribute)) {
						drawAnnotationInterval(gc, drawingArea, delegate,
								interval, selected, printing, overlap);
					}
				}
			}
		} else {
			drawAnnotationInterval(gc, drawingArea, delegate, interval,
					selected, printing, overlap);
		}
	...
	private void drawAnnotationInterval(GC gc, Rectangle drawingArea,
		...
		drawFocus(gc, drawingArea, delegate, interval, selected, printing,
				overlap);
		...
		Pattern pattern = new Pattern(gc.getDevice(), 0, iRect.y, 0, iRect.y
				+ (iRect.height * 2), gc.getDevice().getSystemColor(
				this.gradientStartColor), gc.getDevice().getSystemColor(
		...
		if (_rounding == 0) {
			...
			gc.fillRoundRectangle(iRect.x, iRect.y, iRect.width, iRect.height,
					_rounding, _rounding);
			if (showBorder) {
				gc.drawRoundRectangle(iRect.x, iRect.y, iRect.width,
						iRect.height, _rounding, _rounding);
			}
		}
		if (showText) {
			if (horizontal) {
				...
				SwtGraphicsHelper.drawStringVertical(gc, str, iRect.x + 2,
						iRect.y + 2);
			}
		}
		...
		if (interval instanceof AnnotationIntervalImpl) {
			AnnotationIntervalImpl annotationIntervalImpl = (AnnotationIntervalImpl) interval;
			int count = annotationIntervalImpl.getAnnotation().getAttributes()
					.size();
			...
			if ((count > 0) && (iRect.width > 3)) {
				...
				for (Attribute attribute : annotationIntervalImpl
					...
					Color currentColor = new Color(Display.getDefault(),
							new RGB(Integer.parseInt(rgb[0]),
									Integer.parseInt(rgb[1]),
									Integer.parseInt(rgb[2])));
					gc.setBackground(currentColor);
					gc.fillRectangle(iRect.x + startX, (iRect.y + iRect.height)
							- 2 - h, w, h);
					startX += w;
		...
		if (selected && !printing) {
			...
			if (_rounding == 0) {
				...
				gc.fillRoundRectangle(iRect.x, iRect.y, iRect.width,
						iRect.height, _rounding, _rounding);
			}
			gc.setAlpha(255);
		}
	 ...
	 * 
	 * @see
	 * de.jaret.util.ui.timebars.swt.renderer.TimeBarRenderer#getContainingRectangle
	 * (de.jaret.util.date.Interval, org.eclipse.swt.graphics.Rectangle,
	 * boolean)
	 */
	 ...
	 * @return the i rect
	 */
	protected Rectangle getIRect(boolean horizontal, Rectangle drawingArea,
			boolean overlap) {
		if (horizontal) {
			...
			return new Rectangle(drawingArea.x, y, drawingArea.width - 1,
					height - 1);
		} else {
			...
			return new Rectangle(x, drawingArea.y, width - 1,
	 ...
	 * 
	 * @see de.jaret.util.ui.timebars.swt.renderer.TimeBarRenderer2#
	 * getPreferredDrawingBounds(org.eclipse.swt.graphics.Rectangle,
	 * de.jaret.util.ui.timebars.TimeBarViewerDelegate,
	 * de.jaret.util.date.Interval, boolean, boolean, boolean)
	 */
	public Rectangle getPreferredDrawingBounds(Rectangle intervalDrawingArea,
		...
		if (intervalDrawingArea.width < PREFWIDTH) {
			...
			return new Rectangle(intervalDrawingArea.x - diff,
					intervalDrawingArea.y, intervalDrawingArea.width
							+ (2 * diff), intervalDrawingArea.height);
		} else {
	 ...
	 * 
	 * @see
	 * de.jaret.util.ui.timebars.swt.renderer.TimeBarRenderer#getToolTipText
	...
	public void selectionChanged(IWorkbenchPart part, ISelection selection) {
		if ((part instanceof CodingExplorerViewPart)
				&& (selection instanceof IStructuredSelection)) {
			selectedAttributes.clear();
			if (selection.isEmpty()
					|| (!(((IStructuredSelection) selection).getFirstElement() instanceof AttributeNode))) {
				return;
			}
			@SuppressWarnings("unchecked")
			Iterator<AttributeNode> iterator = ((IStructuredSelection) selection)
					.iterator();
