/**
 * File: SubtitleMediaController.java Copyright (c) 2010 phyo This program is
 ...
 * @author phyot
 */
public class AnnotationsMediaControl extends Composite implements
		IAnnotationMediaControl, ISelectionListener {
	 ...
	 *            the media folder
	 */
	public AnnotationsMediaControl(Composite parent, int style,
			AnnotationSetNode annotationSetNodeValue, IObservableMap mediaMap,
			MediaRootNode mediaFolder) {
		super(parent, style);
		...
		projectAttributeRootNode = ((ProjectNode) annotationSetNode
				.getLastParent()).getProjectAttributeRootNode();
		collectionMediaList = ((CollectionNode) annotationSetNode.getParent()
				.getParent()).getResource().getCollectionMediaList();
		collectionMediaClip = annotationSetNode.getResource()
	...
	private void addAnnotation() {
		...
		if (addedAnnotationInterval.getEnd().diffMilliSeconds(
				addedAnnotationInterval.getBegin()) < MIN_ANNOTATION_INTERVAL_MILLISEC) {
			addedAnnotationInterval.setEnd(addedAnnotationInterval.getBegin()
					.copy().advanceMillis(MIN_ANNOTATION_INTERVAL_MILLISEC));
		}
		...
		annotationItem.setMilliSec(addedAnnotationInterval.getBegin()
				.getMillis());
		annotationItem.setStartTime(DateTimeHelper
				.getMilliFromJaretDate(addedAnnotationInterval.getBegin()));
		annotationItem.setDuration(addedAnnotationInterval.getEnd()
				.diffMilliSeconds(addedAnnotationInterval.getBegin()));
		...
		try {
			annotationSetNode.addAnnotations(annotationsToAdd,
					addedSubjectRow.getSubject());
	...
	public void dispose() {
		...
		annotationSetNode.getResource().removePropertyChangeListener(
				AnnotationSet.PROP_HIDECAPTION, captionHideListener);
		annotationSetNode.getResource().removePropertyChangeListener(
				AnnotationSet.PROP_LOCK, lockAnnotationsListener);
		if (this.selectionService != null) {
			this.selectionService.removeSelectionListener(this);
			this.selectionService
					.removeSelectionListener(this.annotationIntervalRender);
		}
	}
	...
	private void initData() {
		mediaClipStartDate = new JaretDate().setTime(0, 0, 0,
				collectionMediaClip.getStartOffset());
		marker = new TimeBarMarkerImpl(true, mediaClipStartDate.copy());
		
		// Creates a new AnnotationTimeBarModel
		annotationTimeBarModel = new AnnotationTimeBarModel(annotationSetNode,
				marker);
		...
		timeBarViewer.registerTimeBarRenderer(AnnotationIntervalImpl.class,
				annotationIntervalRender);
		timeBarViewer.setMinDate(mediaClipStartDate.copy());
		timeBarViewer.setMaxDate(mediaClipStartDate.copy().advanceMillis(
				(long) collectionMediaClip.getDuration()));
		...
		timeScaleSlider.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (timeScaleSlider.getSelection() == timeScaleSlider
						.getMinimum()) {
					timeBarViewer.setPixelPerSecond((double) ((((Composite) timeBarViewer)
							.getClientArea().width
							- timeBarViewer.getYAxisWidth() - 3) * 1000)
							/ (double) (collectionMediaClip.getDuration()));
				} else {
					timeBarViewer.setPixelPerSecond((double) timeScaleSlider
							.getSelection() / 1000);
				}
				timeBarViewer
						.scrollDateToVisible(marker
								.getDate()
								.copy()
								.advanceMillis(
										(long) ((MARKER_VISIABLE_PIXEL * 1000) / timeBarViewer
												.getPixelPerSecond())));
			}
		});
		...
		captionChangeListener = new ICaptionChangeListener() {
			...
			public void captionChange(Object source, CaptionChangeEvent eventArg) {
				if (!annotationSetNode.getResource().isHideCaption()) {
					if ((eventArg.getCaptionState() == ICaptionChangeListener.CaptionPublishState.SET)
							&& !captionChangeProviders.containsKey(source)) {
						captionChangeProviders.put(source,
								eventArg.getCaptionText());
					}
					if ((eventArg.getCaptionState() == CaptionPublishState.UNSET)
							&& captionChangeProviders.containsKey(source)) {
						captionChangeProviders.remove(source);
					}
					
					StringBuilder captionBuilder = new StringBuilder();
					for (Entry<Object, String> set : captionChangeProviders
							.entrySet()) {
						AnnotationIntervalImpl cI = (AnnotationIntervalImpl) set
								.getKey();
						captionBuilder.append("[");
						captionBuilder.append(cI.getOwner().getSubject()
								.getName());
						captionBuilder.append(": ");
		...
		captionHideListener = new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent arg0) {
				AnnotationsMediaControl.this
						.updateCaptionComposite(annotationSetNode.getResource()
								.isHideCaption());
				if (annotationSetNode.getResource().isHideCaption()) {
		...
		lockAnnotationsListener = new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent arg0) {
				AnnotationsMediaControl.this
						.updateAnnotationTextComposite(annotationSetNode
								.getResource().isLock());
				if (annotationSetNode.getResource().isLock()
						&& (AnnotationsMediaControl.this.addedAnnotationInterval != null)) {
					AnnotationsMediaControl.this.stopCaptionMark();
				}
			}
		};
		
		annotationSetNode.getResource().addPropertyChangeListener(
				AnnotationSet.PROP_HIDECAPTION, captionHideListener);
		annotationSetNode.getResource().addPropertyChangeListener(
				AnnotationSet.PROP_LOCK, lockAnnotationsListener);
		...
		timeBarViewer.registerTimeBarRenderer(MediaClipIntervalImpl.class,
	...
	private void initMediaRow() {
		for (CollectionMedia collectionMedia : collectionMediaList) {
			MediaClipIntervalImpl interval = new MediaClipIntervalImpl(
					collectionMedia,
					((AbstractMedia) _mediaMap.get(collectionMedia)),
					mediaClipStartDate, collectionMediaClip.getDuration(),
					marker, this);
			interval.addTimeAvailableListener(new TimeAvailableListener() {
				
				public void timeAvailableChanged(TimeAvailableEvent eventArg) {
					if (eventArg.getTimeAvailable()
							&& !timeAvailableIntervalList.contains(eventArg
									.getSource())) {
						timeAvailableIntervalList
								.add((MediaClipIntervalImpl) eventArg
										.getSource());
					}
					if (!eventArg.getTimeAvailable()
							&& timeAvailableIntervalList.contains(eventArg
					...
					updateTimeListener();
					if (AnnotationsMediaControl.this.isPlaying()
							&& (timeAvailableIntervalList.size() == 0)) {
			...
			if (interval.isTimeAvailable()) {
				...
				if (currentTimeListener == null) {
					...
					currentTimeListener
							.addTimeChangeListener(mediaTimeChangeListener);
				}
			}
	...
	private void initUI() {
		...
		captionTextView = new Text(captionTextComposite, SWT.MULTI | SWT.WRAP
				| SWT.V_SCROLL | SWT.H_SCROLL | SWT.CENTER);
		captionTextView.setEditable(false);
		captionTextView.setBackground(getShell().getDisplay().getSystemColor(
				SWT.COLOR_BLACK));
		captionTextView.setForeground(getShell().getDisplay().getSystemColor(
				SWT.COLOR_YELLOW));
		captionTextView.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				true));
		...
		textLabelButton.setImage((Image) resourceManager.get(ResourceLoader
				.getIconDescriptor("text.gif")));
		textLabelButton.setLayoutData(new GridData());
		
		barLabelButton = new Button(mediaControls, SWT.TOGGLE);
		barLabelButton.setImage((Image) resourceManager.get(ResourceLoader
				.getIconDescriptor("grouping.gif")));
		barLabelButton.setLayoutData(new GridData());
		
		lockButton = new Button(mediaControls, SWT.TOGGLE);
		lockButton.setImage((Image) resourceManager.get(ResourceLoader
				.getIconDescriptor("lock_open.png")));
		...
		attributesSelectionLinkButton.setImage((Image) resourceManager
				.get(ResourceLoader.getIconDescriptor("layout_link.png")));
		...
		annotationTextWidget = new Text(annotationTextComposite, SWT.MULTI
		...
		attributesSelectionLinkButton.addListener(SWT.Selection,
				new Listener() {
					public void handleEvent(Event event) {
						annotationIntervalRender
								.setShowAttributeSelection(attributesSelectionLinkButton
										.getSelection());
		...
		textLabelButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				annotationIntervalRender.setShowText(textLabelButton
						.getSelection());
		...
		barLabelButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				annotationIntervalRender.setShowBorder(barLabelButton
						.getSelection());
				timeBarViewer.redraw();
			}
		});
	 ...
	 * 
	 * @see
	 * synergyviewcore.subtitle.ui.ISubtitleControl#isMute(synergyviewcore.subtitle
	 * .ui.MediaClipIntervalImpl)
	 */
	...
	private void registerCaptionListeners() {
		for (int i = 0; i < annotationTimeBarModel.getRowCount(); i++) {
			for (Interval interval : annotationTimeBarModel.getRow(i)
					.getIntervals()) {
				if (interval instanceof AnnotationIntervalImpl) {
					...
					addedAnnotationIntervalImpl
							.addCaptionChangeListener(captionChangeListener);
				}
			}
		}
		annotationTimeBarModel
				.addTimeBarModelListener(new TimeBarModelListener() {
					...
					public void elementRemoved(TimeBarModel arg0,
						...
						if (arg2 instanceof AnnotationIntervalImpl) {
							...
							removedAnnotationIntervalImpl
									.removeCaptionChangeListener(captionChangeListener);
						}
					}
	...
	private void registerDraggingListener() {
		marker.addTimeBarMarkerListener(new TimeBarMarkerListener() {
			...
			public void markerMoved(TimeBarMarker arg0, JaretDate arg1,
					JaretDate arg2) {
				if (isMarkerDragging) {
					for (Interval interval : mediaRowModel.getIntervals(marker
							.getDate())) {
					...
					if (marker.getDate().diffMilliSeconds(
							mediaClipStartDate.copy().advanceMillis(
									(long) collectionMediaClip.getDuration())) > 0) {
						marker.setDate(mediaClipStartDate.copy().advanceMillis(
								(long) collectionMediaClip.getDuration()));
					}
				}
				...
				if ((addedAnnotationInterval != null)
						&& (arg2.diffMilliSeconds(addedAnnotationInterval
								.getBegin()
								.copy()
								.advanceMillis(MIN_ANNOTATION_INTERVAL_MILLISEC)) > 0)) {
		...
		timeBarViewer.addTimeBarChangeListener(new ITimeBarChangeListener() {
			...
			public void intervalChanged(TimeBarRow row, Interval interval,
				...
				if (interval instanceof AnnotationIntervalImpl) {
					try {
						annotationSetNode
								.updateAnnotation(((AnnotationIntervalImpl) interval)
										.getAnnotation());
			...
			public void intervalIntermediateChange(TimeBarRow row,
				...
				if (interval instanceof AnnotationIntervalImpl) {
					...
					if (changedInterval.getBegin().diffMilliSeconds(
							mediaClipStartDate) < 0) {
						changedInterval.setBegin(mediaClipStartDate);
					}
					if (changedInterval.getEnd().diffMilliSeconds(
							mediaClipStartDate.copy().advanceMillis(
									(long) collectionMediaClip.getDuration())) > 0) {
						changedInterval.setEnd(mediaClipStartDate.copy()
								.advanceMillis(
										(long) collectionMediaClip
												.getDuration()));
					}
					long duration = changedInterval.getEnd().diffMilliSeconds(
							interval.getBegin());
					if (duration < MIN_ANNOTATION_INTERVAL_MILLISEC) {
						changedInterval
								.setEnd(changedInterval
										.getBegin()
										.copy()
										.advanceMillis(
												MIN_ANNOTATION_INTERVAL_MILLISEC));
					}
				}
			}
	...
	private void registerIntervalModifier() {
		timeBarViewer.addIntervalModificator(new DefaultIntervalModificator() {
			...
			public boolean isShiftingAllowed(TimeBarRow row, Interval interval) {
				return ((row == mediaRowModel) || annotationSetNode
						.getResource().isLock()) ? false : true;
			}
			
			@Override
			public boolean isSizingAllowed(TimeBarRow row, Interval interval) {
				if ((row == mediaRowModel)
						|| annotationSetNode.getResource().isLock()) {
					return false;
				}
				if (interval instanceof AnnotationIntervalImpl) {
					long duration = interval.getEnd().diffMilliSeconds(
							interval.getBegin());
					if (duration >= MIN_ANNOTATION_INTERVAL_MILLISEC) {
						return true;
					} else {
	...
	public void selectionChanged(IWorkbenchPart part, ISelection selection) {
		...
		if (attributesSelectionLinkButton.getSelection()
				&& (part instanceof CodingExplorerViewPart)) {
			...
			while (iterator.hasNext()) {
				...
				for (int i = 0; i < annotationTimeBarModel.getRowCount(); i++) {
					if (annotationTimeBarModel.getRow(i) instanceof SubjectRowModel) {
						AnnotationIntervalImpl impl = ((SubjectRowModel) annotationTimeBarModel
								.getRow(i)).getAnnotationNode((Annotation) obj);
						if (impl != null) {
			...
			if (selected.entrySet().size() == 1) {
				timeBarViewer.getSelectionModel().setSelectedInterval(
						selected.entrySet().iterator().next().getValue());
				timeBarViewer.scrollIntervalToVisible(timeBarViewer
						.getSelectionModel().getSelectedIntervals().get(0));
			} else if (selected.entrySet().size() > 1) {
				for (Map.Entry<Annotation, AnnotationIntervalImpl> entry : selected
						.entrySet()) {
					timeBarViewer.getSelectionModel().addSelectedInterval(
							entry.getValue());
				}
			}
		}
	 ...
	 * 
	 * @see
	 * synergyviewcore.subtitle.ui.ISubtitleControl#setMute(synergyviewcore.
	 * subtitle.ui.MediaClipIntervalImpl, boolean)
	 */
	...
	public void setRowSelection(int number) {
		if (number < annotationTimeBarModel.getRowCount()) {
			timeBarViewer.getSelectionModel().setSelectedRow(
					annotationTimeBarModel.getRow(number));
			annotationTextWidget.setFocus();
		}
	}
	...
	public void setSelectionService(ISelectionService selectionService) {
		...
		this.selectionService
				.addSelectionListener(this.annotationIntervalRender);
	}
	...
	private void setupDND() {
		...
		_target.addDropListener(new DropTargetListener() {
			...
			public void dragOver(DropTargetEvent event) {
				int Y = Display.getCurrent().map(null, timeBarViewer, event.x,
						event.y).y;
				int X = Display.getCurrent().map(null, timeBarViewer, event.x,
						event.y).x;
							...
							if (!selection.isEmpty()) {
								...
								while (i.hasNext()) {
									...
									if (o instanceof AttributeNode) {
										...
										annotationSetNode
												.getAnnotationAttributeController(
														((AnnotationIntervalImpl) il
																.get(0))
																.getAnnotation())
												.addAttributeList(list);
										timeBarViewer.setFocussedInterval(il
												.get(0));
										
										timeBarViewer.getSelectionModel()
												.setSelectedInterval(il.get(0));
										timeBarViewer.fireSelectionChanged();
									}
								}
							}
	...
	public void showSubjectsDialogToAdd() {
		...
		try {
			...
			ElementListSelectionDialog dialog = new ElementListSelectionDialog(
					AnnotationsMediaControl.this.getShell(),
					new NavigatorLabelProvider());
			dialog.setTitle("Select Subjects to add");
			dialog.setMultipleSelection(true);
			ProjectNode projectNode = (ProjectNode) projectAttributeRootNode
					.getLastParent();
	...
	public void startCaptionMark() {
		if (annotationSetNode.getResource().isLock()) {
			...
			messageBox
					.setMessage("Please ensure the annotation set is unlocked.");
			messageBox.open();
			return;
		}
		if (!timeBarViewer.getSelectionModel().isEmpty()
				&& (annotationTextWidget.getText().compareTo("") != 0)
				&& (addedAnnotationInterval == null)) {
			addedSubjectRow = (SubjectRowModel) timeBarViewer
					.getSelectionModel().getSelectedRows().get(0);
			addedAnnotationInterval = new IntervalImpl();
			JaretDate d = marker.getDate().copy();
			addedAnnotationInterval.setBegin(d);
			addedAnnotationInterval.setEnd(d
					.advanceMillis(MIN_ANNOTATION_INTERVAL_MILLISEC));
			addedSubjectRow.addInterval(addedAnnotationInterval);
			timeBarViewer.setFocussedInterval(addedSubjectRow,
					addedAnnotationInterval);
			if (!_isPlaying) {
			...
			messageBox
					.setMessage("Please ensure a subject is selected, the text is not empty.");
			messageBox.open();
		}
	}
	...
	private void updateCaptionComposite(boolean hideCaption) {
		...
		captionButton.setImage((Image) resourceManager.get(ResourceLoader
				.getIconDescriptor(iconName)));
	...
	private void updateScale() {
		timeBarViewer
				.setPixelPerSecond((double) ((((Composite) timeBarViewer)
						.getClientArea().width - timeBarViewer.getYAxisWidth() - 3) * 1000)
						/ (double) (collectionMediaClip.getDuration()));
		timeScaleSlider
				.setMaximum((int) (timeBarViewer.getPixelPerSecond() * 1000 * zoom_times));
		timeScaleSlider
				.setMinimum((int) (timeBarViewer.getPixelPerSecond() * 1000));
		timeScaleSlider
				.setSelection((int) (timeBarViewer.getPixelPerSecond() * 1000));
	}
	...
	private void updateTimeListener() {
		if (currentTimeListener == null) {
			...
			if (!timeAvailableIntervalList.contains(currentTimeListener)) {
				currentTimeListener
						.removeTimeChangeListener(mediaTimeChangeListener);
				if (timeAvailableIntervalList.size() > 0) {
					currentTimeListener = timeAvailableIntervalList.get(0);
					currentTimeListener
							.addTimeChangeListener(mediaTimeChangeListener);
