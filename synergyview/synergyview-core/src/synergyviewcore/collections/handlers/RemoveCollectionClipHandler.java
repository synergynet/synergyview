	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		for (Iterator<?> i = structSel.iterator(); i.hasNext();) {
			...
			if (element instanceof CollectionMediaClipNode) {
				CollectionMediaClip clip = ((CollectionMediaClipNode) element)
						.getResource();
				List<CollectionMediaClip> clipsToRemove = new ArrayList<CollectionMediaClip>();
				clipsToRemove.add(clip);
				try {
					((CollectionNode) ((CollectionMediaClipNode) element)
							.getParent()).removeClip(clipsToRemove);
				} catch (Exception ex) {
					MessageDialog.openError(
							PlatformUI.getWorkbench().getDisplay()
									.getActiveShell(),
							"Error",
							String.format("Unable to delete %s.",
									clip.getClipName()));
				}
			}
		}
