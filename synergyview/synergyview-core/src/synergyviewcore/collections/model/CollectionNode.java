...
public class CollectionNode extends AbstractParent<Collection> {
	...
	public CollectionNode(Collection resourceValue, IParentNode parentValue) {
		...
		eManagerFactory = parentValue.getEMFactoryProvider()
	...
	public void addClip(List<CollectionMediaClip> cClips) throws Exception {
		...
		try {
			...
			for (CollectionMediaClip cClip : cClips) {
				CollectionMediaClipNode node = new CollectionMediaClipNode(
						cClip, CollectionNode.this);
				_children.add(node);
			}
						...
						for (IEditorReference editorRef : pageRef
							...
							try {
								if (editorRef.getId().compareTo(
										CollectionEditor.ID) == 0) {
									NodeEditorInput cNodeEditorInput = (NodeEditorInput) editorRef
											.getEditorInput();
									if (CollectionNode.this == cNodeEditorInput
											.getNode()) {
										pageRef.closeEditor(
	 ...
	 */
	public CollectionMediaClipNode findCollectionMediaClipNode(
			CollectionMediaClip clipValue) {
	...
	public List<String> getChildrenNames() {
		...
		for (INode collectionClipNode : _children) {
			nameList.add(((CollectionMediaClipNode) collectionClipNode)
					.getResource().getClipName());
		}
		return nameList;
	}
	...
	public List<String> getMediaNames() {
		...
		try {
			...
			TypedQuery<String> q = entityManager
					.createQuery(
							"SELECT CM.mediaName FROM CollectionMedia CM Where CM.collection = :collection",
							String.class);
	...
	private void loadChildNodes() {
		for (CollectionMediaClip collectionItem : resource
				.getCollectionMediaClipList()) {
			CollectionMediaClipNode collectionClipNodeFolder = new CollectionMediaClipNode(
					collectionItem, this);
	...
	public void removeMedia(List<CollectionMedia> cMedias) {
		...
		try {
			...
			for (CollectionMedia cMedia : cMedias) {
				CollectionMedia collectionToRemove = entityManager
						.merge(cMedia);
