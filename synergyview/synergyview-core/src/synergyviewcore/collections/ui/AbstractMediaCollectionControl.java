/**
 * File: AbstractMediaCollectionControl.java Copyright (c) 2010 phyo This
...
public abstract class AbstractMediaCollectionControl extends Composite {
	...
	/**
	 * The listener interface for receiving collectionMedia events. The class
	 * that is interested in processing a collectionMedia event implements this
	 * interface, and the object created with that class is registered with a
	 * component using the component's
	 * <code>addCollectionMediaListener<code> method. When
	...
	private PropertyChangeListener mediaChangeListener = new PropertyChangeListener() {
		public void propertyChange(final PropertyChangeEvent arg0) {
			AbstractMediaCollectionControl.this.getDisplay().asyncExec(
					new Runnable() {
						
						public void run() {
							if (arg0.getSource().hashCode() != ((AbstractMedia) _mediaMap
									.get(currentListenedMedia.getId()))
									.hashCode()) {
								return;
							}
							final long newValue = Long.parseLong(arg0
			...
			AbstractMediaCollectionControl.this.getDisplay().asyncExec(
					new Runnable() {
						...
						public void run() {
							...
							final boolean newValue = Boolean.parseBoolean(arg0
									.getNewValue().toString());
	 ...
	 */
	public AbstractMediaCollectionControl(Composite parent, int style,
			IObservableMap mediaMap, CollectionNode collectionNode,
	...
	public void addMedia(MediaNode mediaNode) {
		try {
			...
			for (CollectionMediaListener l : listeners) {
				l.mediaCollectionChanged(new MediaListEvent(this,
						MediaListEvent.CollectionChangeType.MediaAdded,
						collectionMedias));
			}
		} catch (Exception ex) {
	...
	public void addMediaClip() {
		CollectionMediaClipRowModel row = (CollectionMediaClipRowModel) this
				.getTimeBarViewer().getModel().getRow(1);
		...
		if (dialog.open() == Window.OK) {
			...
			try {
				...
				MessageDialog.openError(PlatformUI.getWorkbench().getDisplay()
						...
						String.format("Unable to add %s.", clip.getClipName()));
			}
		}
	}
	...
	protected void attachAbstractMedia() {
		...
		for (CollectionMedia media : collectionNode.getResource()
				.getCollectionMediaList()) {
			AbstractMedia abstractMedia = mediaRootNode.getMediaNode(
					media.getMediaName()).createMediaInstance();
	...
	protected void clearMediaListener() {
		for (CollectionMedia media : this.collectionNode.getResource()
				.getCollectionMediaList()) {
			((AbstractMedia) _mediaMap.get(media.getId()))
					.removePropertyChangeListener(AbstractMedia.PROP_TIME,
							mediaChangeListener);
		}
	}
	...
	public void clearMedias() {
		...
		for (CollectionMediaListener l : listeners) {
			l.mediaCollectionChanged(new MediaListEvent(this,
					MediaListEvent.CollectionChangeType.MediaRemoved,
					this.collectionNode.getResource().getCollectionMediaList()));
		}
	...
	protected CollectionMedia getEarlistMedia() {
		...
		if (this.collectionNode.getResource().getCollectionMediaList().size() > 0) {
			earlistMedia = this.collectionNode.getResource()
					.getCollectionMediaList().get(0);
		}
		for (CollectionMedia media : this.collectionNode.getResource()
				.getCollectionMediaList()) {
	...
	// TODO This is used in many places, may be this can be moved as a static
	// method under media.Util class
	/**
	 * Gets the formatted time.
	 * 
	 * @param time
	...
	protected long getTime() {
		for (CollectionMedia media : this.collectionNode.getResource()
				.getCollectionMediaList()) {
			int mediaTime = ((AbstractMedia) _mediaMap.get(media.getId()))
					.getTime();
			if ((mediaTime > 0)
					&& (mediaTime < ((AbstractMedia) _mediaMap.get(media
							.getId())).getDuration())) {
	...
	protected void playMedia(long currentTime) {
		for (CollectionMedia media : this.collectionNode.getResource()
				.getCollectionMediaList()) {
			if ((currentTime >= media.getOffSet())
					&& (currentTime <= (media.getOffSet() + ((AbstractMedia) _mediaMap
							.get(media.getId())).getDuration()))) {
				((AbstractMedia) _mediaMap.get(media.getId()))
						.setTime((int) (currentTime - media.getOffSet()));
	...
	public void removeMedia(CollectionMedia media) {
		try {
			// if (!_collection.getCollectionMediaList().contains(media))
			// return;
			//
			// this.stopMedia();
			//
			// //TODO Why remove all listeners, this is also called in
			// updateMediaListener!
			clearMediaListener();
			//
			// //TODO What if currentListenedMedia is assign to other media and
			// not the one that is to be removed
			this.currentListenedMedia = null;
			//
			...
			// Do we need to update removeMedia method to accept multiple media
			// objects
	...
	public void removeMediaClip(CollectionMediaClip mediaClip) {
		...
		try {
			...
			MessageDialog.openError(PlatformUI.getWorkbench().getDisplay()
					.getActiveShell(), "Error",
					String.format("Unable to delete media clips."));
		}
	...
	protected void setTime(long time) {
		...
		for (CollectionMedia media : this.collectionNode.getResource()
			...
			} else if (time > (media.getOffSet() + ((AbstractMedia) _mediaMap
					.get(media.getId())).getDuration())) {
				((AbstractMedia) _mediaMap.get(media.getId()))
						.setTime(((AbstractMedia) _mediaMap.get(media.getId()))
								.getDuration());
			} else {
				((AbstractMedia) _mediaMap.get(media.getId()))
						.setTime((int) (time - media.getOffSet()));
			}
		}
	}
	...
	protected void updateDuration() {
		...
		for (CollectionMedia media : this.collectionNode.getResource()
			...
			if (_mediaMap.containsKey(media.getId())) {
				long currentDuration = ((AbstractMedia) _mediaMap.get(media
						.getId())).getDuration() + media.getOffSet();
	...
	protected void updateMediaListener(long time) {
		...
		if ((this.currentListenedMedia != null)
				&& (time <= (this.currentListenedMedia.getOffSet() + ((AbstractMedia) _mediaMap
						.get(currentListenedMedia.getId())).getDuration()))
				&& (time > currentListenedMedia.getOffSet())) {
			return;
		}
		...
		for (CollectionMedia media : this.collectionNode.getResource()
				.getCollectionMediaList()) {
			if (_mediaMap.containsKey(media.getId())) {
				if ((time <= (media.getOffSet() + ((AbstractMedia) _mediaMap
						.get(media.getId())).getDuration()))
						&& (time > media.getOffSet())) {
					mediaListenTo = media;
					break;
				}
			}
		}
		((AbstractMedia) _mediaMap.get(mediaListenTo.getId()))
				.addPropertyChangeListener(AbstractMedia.PROP_TIME,
						mediaChangeListener);
		((AbstractMedia) _mediaMap.get(mediaListenTo.getId()))
				.addPropertyChangeListener(AbstractMedia.PROP_TIME,
						mediaPlayingStatusChangeListener);
