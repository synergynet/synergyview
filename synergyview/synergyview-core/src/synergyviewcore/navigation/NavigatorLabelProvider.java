 ...
 */
public class NavigatorLabelProvider extends ColumnLabelProvider implements
	...
	private IMapChangeListener mapChangeListener = new IMapChangeListener() {
		...
		public void handleMapChange(MapChangeEvent event) {
			...
			LabelProviderChangedEvent newEvent = new LabelProviderChangedEvent(
					NavigatorLabelProvider.this, affectedElements.toArray());
	...
	public NavigatorLabelProvider(IObservableSet knownElements) {
		setObserable(Properties.observeEach(knownElements,
				BeanProperties.values(new String[] { "label" })));
	}
	...
	public StyledString getStyledText(Object element) {
		...
		if (element instanceof CollectionNode) {
			...
			styledString.append(String.format(" (%s / %s)",
					((CollectionNode) element).getResource()
							.getCollectionMediaClipList().size(),
					((CollectionNode) element).getResource()
							.getCollectionMediaList().size()),
			...
			if (aElement.getResource().isLock()) {
				styledString
						.append(" (Completed)", StyledString.COUNTER_STYLER);
			}
