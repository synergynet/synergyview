...
public class SubjectPropertyPage extends PropertyPage {
	...
	private void addSectionDetails(Composite composite) {
		...
		collectionModelBindingCtx.bindValue(SWTObservables.observeText(
				subjectNameText, SWT.Modify), BeansObservables.observeValue(
						subject, Subject.PROP_NAME), new UpdateValueStrategy(
		...
		// Session text field
		subjectDescText = new Text(composite, SWT.WRAP | SWT.BORDER
				| SWT.MULTI);
		gridData = new GridData();
		gridData.horizontalAlignment = SWT.FILL;
		gridData.grabExcessHorizontalSpace = true;
