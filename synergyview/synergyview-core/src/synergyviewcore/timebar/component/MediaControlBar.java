...
public class MediaControlBar extends Composite {
	...
	/**
	 * The listener interface for receiving mediaControl events. The class that
	 * is interested in processing a mediaControl event implements this
	 * interface, and the object created with that class is registered with a
	 * component using the component's
	 * <code>addMediaControlListener<code> method. When
	...
	protected LocalResourceManager resourceManager = new LocalResourceManager(
	 ...
	 * 
	 * @param mediaControlListener
	 *            the media control listener
	 */
	...
	protected void createControls(Composite parent) {
		...
		playButton.setImage((Image) resourceManager.get(ResourceLoader
		...
		playButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				if (playButton.getSelection()) {
					playButton.setImage((Image) resourceManager.get(ResourceLoader
							.getIconDescriptor("control_pause_blue.png")));
		...
		muteButton.setImage((Image) resourceManager.get(ResourceLoader
				.getIconDescriptor("sound.png")));
		muteButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				if (muteButton.getSelection()) {
					muteButton.setImage((Image) resourceManager
							.get(ResourceLoader
									.getIconDescriptor("sound_mute.png")));
				} else {
					muteButton.setImage((Image) resourceManager
							.get(ResourceLoader.getIconDescriptor("sound.png")));
				}
		...
		lockButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				if (lockButton.getSelection()) {
					lockButton.setImage((Image) resourceManager
							.get(ResourceLoader.getIconDescriptor("lock.png")));
				} else {
					lockButton.setImage((Image) resourceManager
							.get(ResourceLoader
									.getIconDescriptor("lock_open.png")));
	 ...
	 * 
	 * @param enabled
	 *            the new lock button enabled
	 */
	public void setLockButtonEnabled(boolean enabled) {
		if (enabled) {
			lockButton.setImage((Image) resourceManager.get(ResourceLoader
					.getIconDescriptor("lock.png")));
			lockButton.setSelection(true);
		} else {
			lockButton.setImage((Image) resourceManager.get(ResourceLoader
					.getIconDescriptor("lock_open.png")));
